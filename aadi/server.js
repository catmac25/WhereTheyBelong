...
// Configure SendGrid (used for public case email alerts)
...
const ALERT_RECIPIENTS = (process.env.ALERT_RECIPIENTS || "")
  ...
  .map((e) => e.trim())
  .filter(Boolean);
const ALERT_FROM_EMAIL = process.env.ALERT_FROM_EMAIL || "no-reply@example.com";
...
async function buildPublicCasePdf(payload, caseId) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(18).text("New Public Missing Person Case", { underline: true });
      doc.moveDown();
      doc.fontSize(12);
      ...
      doc.text(`Birth marks: ${payload.birth_marks || "-"}`);
...
async function sendPublicCaseEmail(caseId, payload) {
  if (!process.env.SENDGRID_API_KEY || !ALERT_RECIPIENTS.length) {
    console.log(
      "SendGrid not configured in node_proxy. " +
      "Set SENDGRID_API_KEY and ALERT_RECIPIENTS in backend/node_proxy/.env to enable public case emails."
    );
    return;
  }

  try {
    const pdfBuffer = await buildPublicCasePdf(payload, caseId);

    const msg = {
      to: ALERT_RECIPIENTS,
      from: ALERT_FROM_EMAIL,
      subject: `New public missing persons case: ${payload.name || caseId}`,
      html: `
        <p>A new public missing person case has been registered.</p>
        <ul>
          <li><strong>Case ID</strong>: ${caseId}</li>
          <li><strong>Name</strong>: ${payload.name || "-"}</li>
          <li><strong>Mobile</strong>: ${payload.mobile_number || "-"}</li>
          <li><strong>Email</strong>: ${payload.email || "-"}</li>
          <li><strong>Address</strong>: ${payload.address || "-"}</li>
          <li><strong>Birth marks</strong>: ${payload.birth_marks || "-"}</li>
        </ul>
        <p>See attached PDF for a printable summary.</p>
      `,
      attachments: [
        {
    ...
    console.log("SendGrid: notification email sent for public case", caseId);
  } catch (err) {
    console.error("SendGrid: failed to send email for case", caseId, err.message);
  }
}
...
async function sendRegisteredCaseEmail(caseId, payload) {
  ...
  try {
    ...
    console.error("SendGrid: failed to send email for registered case", caseId, err.message);
  }
}

// Test-only endpoint: send a registered-case email without touching the Python/DB layer.
// This lets you verify SendGrid + PDF end-to-end even if the FastAPI service is returning 503.
...
app.post('/api/publicsubmission', upload.single('image'), async (req, res) => {
  try {
    ...
    // fire-and-forget email; don't delay HTTP response
    if (caseId) {
      (async () => {
        try {
          await sendPublicCaseEmail(caseId, {
            name: req.body.name,
            mobile_number: req.body.mobile_number,
            email: req.body.email,
            address: req.body.address,
            birth_marks: req.body.birth_marks,
          });
        } catch (err) {
          console.error("Error while sending public case email from node_proxy:", err.message);
        }
      })();
    }
