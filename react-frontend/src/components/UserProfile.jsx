...
const UserProfile = () => {
      ...
      const fetchProfile = async () => {
        try {
          ...
          const res = await fetch(
            `${API_BASE_URL}/user-by-email?email=${encodeURIComponent(user.email)}`
          ); // 👈 goes to Node proxy
    ...
    useEffect(()=>{
        const fetchCasesCount = async () => {
            if (!user || !user.email) return;
            try{
                const res = await fetch(`${API_BASE_URL}/registercount?submitted_by=${encodeURIComponent(user.email)}&status=NF`);
                const data = await res.json();
